...
# …and the remaining 8 languages (ja, zh, hi, de, es, fr, pt, ru) —
# every i18n block must carry all 10 supported languages
</i18n>
```
...
- **Dynamic Styles:** Bind component state to styles with `:style` + `var()` custom properties. Do **not** use `v-bind()` in `<style>` (matches AGENTS.md; the codebase contains no `v-bind()` styles).
