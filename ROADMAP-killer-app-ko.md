...
## Phase 3 — 레트로 (방향 변경, 2026-09-26)

> **스킨 기능은 폐기**(기현님 결정 — 구상과 달라 되돌림, `f21fd54d`). 대신 **색 테마를 유명 계산기 색으로 정리**:
> 기본 = Braun ET66 풍, 골드 파이낸셜(HP-12C) · 레드 LED(HP-35) · 스쿨 네이비(카시오 fx) · 청록 형광(카시오 VFD) · 초콜릿 LED(TI-30) · 이탈리안 옐로(올리베티 Divisumma 18), 기존 잿빛 · 고대비 유지. 이름은 묘사형(모델명은 코드 주석에만). 아래 원래 계획은 이력용.

### (이력) Phase 3 — 레트로 스킨 (0.15~0.16, 약 2주)
