...
- **수식 계산기 모드**: [mathjs](https://mathjs.org/) 문법을 사용하여 수학 수식을 직접 입력하고 계산할 수 있는 새로운 계산기 모드 추가. 사칙연산, 괄호, 내장 함수(`sin`, `cos`, `sqrt`, `log`, `ln`, `abs`, `round`, `nthRoot` 등), 상수(`pi`, `e`, `phi`)를 지원.
  - 수식에서 `@`로 현재 계산값을, `$`로 저장된 메모리 값을 참조 가능.
  - 인라인 수식 편집기(Space 키)로 수식을 직접 편집 가능.
  - 내장 도움말 메뉴에서 사용 가능한 함수, 상수, 플레이스홀더 목록 및 mathjs 문서 링크 제공.
  - 수식 계산 결과가 계산 기록에 저장되며, 결과 필드에 전체 수식이 표시됨.
  - 시프트 기능 버튼으로 표준 메모리 연산(MC, MR, MS, M+, M−, M×, M÷) 모두 지원.
- **Flatpak 빌드 지원**: 더 넓은 리눅스 데스크톱 배포를 위한 Flatpak 패키징 지원 추가.
- **자동화된 테스트**: 핵심 수학 연산에 대한 단위 테스트 프레임워크(Vitest) 추가.
- **CI/CD 파이프라인**: 푸시할 때마다 자동으로 린트 및 테스트를 실행하는 GitHub Actions 워크플로우 추가.
- **커밋 전 품질 검사**: 커밋 전 자동 코드 포맷팅 및 린트 검사 추가.

### 변경됨

- **Electron 35 → 40 업그레이드**: 최신 Electron 런타임으로 데스크톱 앱 성능 및 호환성 향상.
- **보안 강화**: 프로덕션 빌드에 더 엄격한 콘텐츠 보안 정책(CSP) 적용.
- **의존성 업데이트**: 모든 의존성을 최신 버전으로 업그레이드하고 Capacitor 패키지 보안 수정 적용.

### 고쳐짐

- **Electron 데스크톱 앱 시작 오류**: 일부 시스템에서 모듈 형식 충돌로 인한 시작 오류 수정.
- **타입 안전성 개선**: 여러 컴포넌트의 타입 체크 오류를 해결하여 안정성 향상.
