...
**Wayland users note**: GNOME/KDE Wayland sessions suffer from upstream bugs where `setTitle` does not repaint the CSD header bar and `setAlwaysOnTop` is a no-op ([tauri#13749](https://github.com/tauri-apps/tauri/issues/13749), [tauri#3117](https://github.com/tauri-apps/tauri/issues/3117)). `configure_gdk_backend()` in `src-tauri/src/lib.rs` can force `GDK_BACKEND=x11` to work around both, but this was found to cause an intermittent WebKitGTK crash on real Wayland hardware, so it defaults to off (native Wayland). If you need the two features and accept the crash risk, opt in with `QCALC_FORCE_XWAYLAND=1 yarn dev:tauri`.

**NVIDIA proprietary driver GPU rendering**: WebKitGTK disables its DMA-BUF renderer on the NVIDIA proprietary driver, dropping hardware acceleration to `Never` (CPU painting) and making animations far choppier than Electron. When NVIDIA is detected, `configure_webkit_gpu()` restores GPU rendering with `WEBKIT_FORCE_DMABUF_RENDERER=1` + `WEBKIT_DMABUF_RENDERER_FORCE_SHM=1` (measured 9.7 -> 47fps during tab switching). Set `QCALC_NO_GPU_FORCE=1` to turn it off if rendering breaks; explicit `WEBKIT_*` renderer variables are respected.

The same function also handles the reverse: the AppImage bundle unconditionally exports `GDK_BACKEND=x11` from the AppRun hook generated by linuxdeploy-plugin-gtk, which makes the crash above a certainty on a Wayland session (reproduced with the 0.13.0 release AppImage). The hook is a build artifact we cannot edit, so the backend is reset to native Wayland before GTK initialises.
