...
<p align="center">
  ...
  <a href="https://github.com/from104/qcalc/releases/latest"><img alt="version 0.13.4" src="https://img.shields.io/badge/version-0.13.4-informational"></a>
...
| **통화 변환** | 340개 통화(법정, 암호화폐, 귀금속) — API 키 불필요                    | `Ctrl+3` |
...
> **0.12.x 이하(Electron)에서 올라오는 경우** Windows와 AppImage는 구버전 앱의 업데이트 확인으로 0.13.1을 받아 설치할 수 있고, Windows에서는 그 과정에서 예전 설치가 함께 제거됩니다. 나머지 패키지(`.deb`·`.rpm`·Snap·Flatpak)는 직접 내려받아 설치해야 합니다. 어느 쪽이든 구버전에서 기록(CSV)과 설정(JSON)을 내보내고 새 앱의 첫 실행 화면(또는 설정)에서 가져오세요.
>
> 0.13.0 자체는 구버전 업데이터가 읽는 파일 없이 배포됐습니다. 따라서 아직 0.12.x를 쓰고 계시다면 0.13.1부터 안내를 받게 됩니다.
