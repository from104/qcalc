...
export class Calculator {
  private previousNumber!: string;  // 이전 계산에 사용된 숫자
  private repeatedNumber!: string;  // 연속 계산시 반복되는 숫자
  private currentNumber!: string;   // 현재 입력/표시 중인 숫자
   ...
   * @param history - 히스토리에 추가할 계산 결과 스냅샷 객체
  ...
  private shiftHistory(): void {
    // 결과 스냅샷을 기본 상태로 초기화
    this.resultSnapshot = {
      previousNumber: '0', // 이전 숫자를 0으로 설정
      operator: '', // 연산자를 빈 문자열로 설정
      argumentNumber: '0', // 인수 숫자를 0으로 설정 
      resultNumber: '0', // 결과 숫자를 0으로 설정
    };
  ...
  private performPreCalculation(): void {
    // 계산에 사용할 숫자 결정
    const numberForCalc = this.shouldReset 
      ? this.repeatedNumber 
      : this.currentNumber;
  ...
  private calculateResult(numberForCalc: string): string {
    ...
    switch (this.currentOperator) {
      case Operator.Plus:
        return prev.add(curr).toString(); // 덧셈 연산
      case Operator.Minus:
        return prev.sub(curr).toString(); // 뺄셈 연산
      case Operator.Mul:
        return prev.mul(curr).toString(); // 곱셈 연산
      case Operator.Div:
        if (curr.isZero()) throw new Error('Cannot divide by zero.'); // 0으로 나누기 방지
        return prev.div(curr).toString(); // 나눗셈 연산
      case Operator.Pow:
        return prev.pow(curr).toString(); // 거듭제곱 연산
      case Operator.Root:
        return prev.pow(MathB.bignumber(1).div(curr)).toString(); // 루트 연산
      case Operator.Mod:
        return prev.mod(curr).toString(); // 나머지 연산
      default:
  ...
  public rec(): void {
    if (Number(this.currentNumber) !== 0) {
      this.currentNumber = this.calculateAndAddHistory(
        this.currentNumber,
        Operator.Rec,
   ...
   * => "8" 반환
   */
  private calculateAndAddHistory(
    number: string,
    operator: Operator,
    calculation: () => string
  ): string {
  ...
  public memoryPlus(): void {
    if (!this.isMemoryReset) {
      this.memoryNumber = MathB.bignumber(this.memoryNumber)
        .add(this.currentNumber)
        .toString();
      this.shouldReset = true;
    }
  }
