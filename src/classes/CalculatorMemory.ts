/**
 * @file CalculatorMemory.ts
 * @description 이 파일은 계산기의 메모리 기능을 관리하는 클래스를 정의합니다.
 *              메모리 저장, 불러오기, 연산 등의 기능을 제공하며, 계산기의
 *              메모리 관련 작업을 처리합니다. 메모리에 숫자를 저장하고 불러오는
 *              기본적인 기능부터 메모리 값을 이용한 사칙연산까지 지원합니다.
 */
