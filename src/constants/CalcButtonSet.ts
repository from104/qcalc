...
export function createCalcButtonSet(t: ComposerTranslation) {
  ...
  const standardExtendedFunctions: ExtendedButtonFunction = {
    ...
    d1: [ 'MR', ['Shift+Backspace', 'Shift+Control+r'], () => { calc.memory.recall(); displayMemoryStatus(); }, false ],
    a2: ['10ⁿ', ['Shift+Control+a'], () => calc.exp10(), false],
    b2: ['x%y', ['Shift+Control+s'], () => calc.mod(), false],
    c2: ['x!', ['Shift+Control+d'], () => calc.fct(), false],
    d2: [ 'M÷', ['Shift+Slash', 'Shift+NumpadDivide'], () => { calc.memory.div(calc.currentNumber); displayMemoryStatus(); }, false ],
    ...
    d6: [ 'MS', ['Shift+Equal', 'Shift+Enter', 'Shift+NumpadEnter'], () => { calc.memory.save(calc.currentNumber); displayMemoryStatus(); }, false ],
  };
